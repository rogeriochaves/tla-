...
\setboolean{shading}{true}
 ...
 ReportProcessorCommittingAtTheEnd}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\EXTENDS} Integers ,\, Sequences ,\, TLC ,\, Helpers}%
\@pvspace{8.0pt}%
 \@x{ {\VARIABLE} payReportQueue ,\, processed ,\, memoryDb ,\, retryQueue ,\,
 action}%
\@pvspace{8.0pt}%
\@x{ PayReportItems \.{\defeq} {\langle} 1 ,\, 2 ,\, 3 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq}}%
 \@x{\@s{8.2} action \.{\in} \{\@w{consuming\_pay\_report}
 ,\,\@w{processing\_successfull} ,\,\@w{put\_in\_memory\_db}
 ,\,\@w{put\_in\_retry\_queue} \}}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq} payReportQueue \.{=} PayReportItems}%
\@x{\@s{16.81} \.{\land}\@s{7.77} processed \.{=} {\langle} {\rangle}}%
\@x{\@s{16.81} \.{\land}\@s{7.77} memoryDb \.{=} {\langle} {\rangle}}%
\@x{\@s{16.81} \.{\land}\@s{7.77} retryQueue \.{=} {\langle} {\rangle}}%
\@x{\@s{16.81} \.{\land}\@s{7.77} action \.{=}\@w{consuming\_pay\_report}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Function to add the item to the processed list, unless it\mbox{'}s already
 there, simulating idempotency
}%
\@xx{}%
 \@x{ ProcessItem ( item ) \.{\defeq} {\IF} item \.{\notin} SeqToSet (
 processed )}%
\@x{\@s{99.27} \.{\THEN} Append ( processed ,\, item )}%
\@x{\@s{99.27} \.{\ELSE} processed}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 When a kafka message is processed successfully, we mark it as successfull,
 unless there is already
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 a previous message in memory db, in this case we never process it, it will
 go to memory db as well
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{ ConsumePayReportSucc \.{\defeq} payReportQueue \.{\neq} {\langle}
 {\rangle}}%
\@x{\@s{105.74} \.{\land}\@s{7.77} memoryDb \.{=} {\langle} {\rangle}}%
\@x{\@s{105.74} \.{\land}\@s{7.77} action \.{=}\@w{consuming\_pay\_report}}%
 \@x{\@s{105.74} \.{\land}\@s{7.77} action \.{'}
 \.{=}\@w{processing\_successfull}}%
 \@x{\@s{105.74} \.{\land}\@s{7.77} processed \.{'} \.{=} ProcessItem ( Head (
 payReportQueue ) )}%
 \@x{\@s{105.74} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} payReportQueue ,\,
 memoryDb ,\, retryQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 When a kafka message is processed with error, we put the message in retry
 queue
}%
\@xx{}%
 \@x{ ConsumePayReportError \.{\defeq} payReportQueue \.{\neq} {\langle}
 {\rangle}}%
 \@x{\@s{110.61} \.{\land}\@s{7.77} Head ( payReportQueue ) \.{\notin}
 SeqToSet ( memoryDb )}%
\@x{\@s{110.61} \.{\land}\@s{7.77} action \.{=}\@w{consuming\_pay\_report}}%
 \@x{\@s{110.61} \.{\land}\@s{7.77} action \.{'}
 \.{=}\@w{put\_in\_memory\_db}}%
 \@x{\@s{110.61} \.{\land}\@s{7.77} memoryDb \.{'} \.{=} Append ( memoryDb ,\,
 Head ( payReportQueue ) )}%
 \@x{\@s{110.61} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} processed ,\,
 payReportQueue ,\, retryQueue {\rangle}}%
\@pvspace{16.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ PutInRetryQueue \.{\defeq} action \.{=}\@w{put\_in\_memory\_db}}%
 \@x{\@s{77.07} \.{\land}\@s{7.77} action \.{'}
 \.{=}\@w{put\_in\_retry\_queue}}%
 \@x{\@s{77.07} \.{\land}\@s{7.77} retryQueue \.{'} \.{=} Append ( retryQueue
 ,\, Head ( payReportQueue ) )}%
 \@x{\@s{77.07} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} processed ,\,
 memoryDb ,\, payReportQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 We commit to kafka either after message is processed successfully OR after
 is had been put on the retry queue
}%
\@xx{}%
 \@x{ CommitToPayReport \.{\defeq} ( action \.{=}\@w{processing\_successfull}
 \.{\lor} action \.{=}\@w{put\_in\_retry\_queue} )}%
 \@x{\@s{92.84} \.{\land}\@s{7.77} action \.{'}\@s{1.08}
 \.{=}\@w{consuming\_pay\_report}}%
 \@x{\@s{92.84} \.{\land}\@s{7.77} payReportQueue \.{'} \.{=} Tail (
 payReportQueue )}%
 \@x{\@s{92.84} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} processed ,\,
 memoryDb ,\, retryQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 We consume messaged from retry queue in parallel, it is only processed if
 sucessfully if the current item in the queue
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 is the same as the \ensuremath{Head} of memory db, if so, the item is taken
 out of the retry queue and memory db, and added to processed
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 list, if not, \ensuremath{ConsumeRetryQueueError} will be triggered and the
 item will end up in the end of the queue again
}%
\@xx{}%
 \@x{ ConsumeRetryQueueSucc \.{\defeq} retryQueue \.{\neq} {\langle}
 {\rangle}}%
 \@x{\@s{111.74} \.{\land}\@s{7.77} Head ( retryQueue ) \.{=} Head ( memoryDb
 )}%
 \@x{\@s{111.74} \.{\land}\@s{7.77} processed \.{'} \.{=} ProcessItem ( Head (
 retryQueue ) )}%
\@x{\@s{111.74} \.{\land}\@s{7.77} memoryDb \.{'} \.{=} Tail ( memoryDb )}%
 \@x{\@s{111.74} \.{\land}\@s{7.77} retryQueue \.{'} \.{=} Tail ( retryQueue
 )}%
 \@x{\@s{111.74} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} action ,\,
 payReportQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 We consume messaged from retry queue in parallel, if there is an error, the
 message goes back to the end of the retry queue
}%
\@xx{}%
 \@x{ ConsumeRetryQueueError \.{\defeq} retryQueue \.{\neq} {\langle}
 {\rangle}}%
 \@x{\@s{116.60} \.{\land}\@s{7.77} retryQueue \.{'} \.{=} Append ( Tail (
 retryQueue ) ,\, Head ( retryQueue ) )}%
 \@x{\@s{116.60} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} processed ,\,
 action ,\, memoryDb ,\, payReportQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 At any time, the consumer may panic in the middle and restart the process
 from the beginning
}%
\@xx{}%
\@x{ PanicConsumer \.{\defeq} action \.{'} \.{=}\@w{consuming\_pay\_report}}%
 \@x{\@s{70.59} \.{\land}\@s{7.77} {\UNCHANGED} {\langle} processed ,\,
 payReportQueue ,\, memoryDb ,\, retryQueue {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} ConsumePayReportSucc}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} ConsumePayReportError}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} PutInRetryQueue}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} ConsumeRetryQueueSucc}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} ConsumeRetryQueueError}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} CommitToPayReport}%
\@x{\@s{20.94} \.{\lor}\@s{7.77} PanicConsumer}%
\@pvspace{8.0pt}%
\@x{ ProcessedCompleteAndInOrderInvariant \.{\defeq}}%
\@x{\@s{16.4} {\IF} Len ( processed ) \.{>} 0 \.{\THEN}}%
\@x{\@s{32.65}}%
\@y{\@s{0}%
 sequences are 1-indexed
}%
\@xx{}%
 \@x{\@s{32.65} \.{\LET} F [ i \.{\in} 1 \.{\dotdot} ( Len ( processed ) \.{+}
 1 ) ] \.{\defeq}}%
 \@x{\@s{53.05} {\IF} i \.{>} 1 \.{\land} {\lnot} F [ i \.{-} 1 ] \.{\THEN}
 {\FALSE}}%
\@x{\@s{140.12} \.{\ELSE} processed [ i ] \.{=} PayReportItems [ i ]}%
\@x{\@s{32.65} \.{\IN} F [ Len ( processed ) ]}%
\@x{\@s{16.4} \.{\ELSE} {\TRUE}}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Feb} 09 11:56:08 \ensuremath{CET} 2022 by
 \ensuremath{rchavesferna
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Feb} 03
 19:11:04 \ensuremath{CET} 2022 by \ensuremath{rchavesferna
}%
\end{cpar}%
\end{lcom}%
\end{document}
